...
let score=0, lives=3, invuln=0, shake=0, slowTime=0, combo=0;
...
function gameOver(){
    state='gameover'; startBtn.style.display='inline-block'; startBtn.textContent='Restart';
...
function spawnBlock(){ 
    ...
    let bx;
    do { bx=Math.random()*(W-bw); } while(Math.abs(bx+bw/2-(player.x+player.w/2))<50);
...
function hit(){
    if(invuln>0) return;
    lives--; invuln=1200; shake=15; combo=0; updateHud();
    player.y-=10;
    if(lives<=0) gameOver();
...
function createParticle(x,y,color){
    for(let i=0;i<8;i++){
        particles.push({x,y,vx:(Math.random()-0.5)*2,vy:(Math.random()-0.5)*2,life:300+Math.random()*200,color});
    }
}
...
function loop(ts){
    ...
    for(let i=blocks.length-1;i>=0;i--){
        const b=blocks[i]; b.y+=b.spd*timeFactor*(dt/16);
    ...
    grad.addColorStop(0,'#021021'); grad.addColorStop(1,'#001016');
